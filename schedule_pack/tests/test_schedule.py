...
from schedule_pack import Schedule, ScheduleItem, Task
from schedule_pack.abstract_schedule import AbstractSchedule
...
class TestSchedule(unittest.TestCase):
    ...
    def test_none_tasks(self):
        ...
        with self.assertRaises(ScheduleArgumentError) as error:
            Schedule(None, 1)
        self.assertEqual(ErrMessage.ERR_TASKS_NOT_LIST_MSG, str(error.exception))

    def test_empty_tasks(self):
        """Проверяет выброс исключения при передаче пустого списка задач."""
        with self.assertRaises(ScheduleArgumentError) as error:
            Schedule([], 1)
        self.assertEqual(ErrMessage.ERR_TASKS_EMPTY_LIST_MSG, str(error.exception))
    ...
    def test_tasks_contains_non_task(self):
        ...
        with self.assertRaises(ScheduleArgumentError) as error:
            Schedule(tasks, 1)
        self.assertEqual(
            ErrTemplate.ERR_INVALID_TASK_TEMPL.format(2), str(error.exception)
        )
    ...
    def test_not_int_executor_idx(self):
        ...
        for idx in incorrect_idx:
            with self.subTest(idx=idx):
                ...
                self.assertEqual(
                    ErrMessage.ERR_EXECUTOR_NOT_INT_MSG, str(error.exception)
                )
        ...
        for idx in incorrect_idx:
            with self.subTest(idx=idx):
                with self.assertRaises(ScheduleArgumentError) as error:
                    schedule.get_schedule_for_executor(idx)
                self.assertEqual(
                    ErrMessage.ERR_EXECUTOR_OUT_OF_RANGE_MSG, str(error.exception)
                )
    ...
    def test_single_task_double_executor(self):
        ...
        executor1_schedule = [ScheduleItem(task_a, 0, task_a.duration)]
        executor2_schedule = [ScheduleItem(None, 0, task_a.duration, is_downtime=True)]
    ...
    def test_double_task_double_executor(self):
        ...
        executor2_schedule = [
            ...
            ScheduleItem(None, 1, 1, is_downtime=True),
        ]
        self.assertEqual(2, schedule.executor_count)
    ...
    def test_triple_task_triple_executor(self):
        ...
        executor2_schedule = [ScheduleItem(task_c, 0, task_c.duration)]
        executor3_schedule = [ScheduleItem(None, 0, 6, is_downtime=True)]
    ...
    def test_triple_has_large_task_triple_executor(self):
        ...
        executor2_schedule = [
            ...
            ScheduleItem(None, 2, 8, is_downtime=True),
        ]
        executor3_schedule = [ScheduleItem(None, 0, 10, is_downtime=True)]
    ...
    def test_complex(self):
        ...
        executor5_schedule = [
            ...
            ScheduleItem(None, 7, 10, is_downtime=True),
        ]
